...
- **Primary Standard**: "Veterinary Clinical Pathology: A Case-Based Approach", WSAVA Spectrum of Care Guidelines, ISCAID Pyoderma, AAHA Oncology, AAFP Hyperthyroidism & Senior Care Guidelines.
...
- 呼叫內建的 Python 腳本 `scripts/rank_diagnostics.py`。
  ...
  ```
- 支援的情境 (Presentations)：
  - `acute_vomiting_diarrhoea` (急性嘔吐與下痢 - BSAVA / Triage)
  - `severe_anaemia_pallor` (嚴重貧血與蒼白 - ClinPath)
  - `pu_pd_polyuria_polydipsia` (多渴多尿 - IRIS / AAHA Diabetes)
  - `icterus_hepatobiliary` (黃疸與肝膽異常 - WSAVA Liver)
  - `pruritus_skin_lesions` (皮膚搔癢/結節 - ISCAID Pyoderma / AAHA Allergic)
  - `skin_mass_nodule` (皮膚腫塊 - AAHA Oncology)
  - `feline_weight_loss_polyphagia` (高齡貓體重流失 - AAFP Hyperthyroidism & Senior)
  - `acute_collapse_seizures` (急性休克與癲癇 - Triage & ClinPath)
  - `cavitary_effusion` (體腔積液/胸水腹水 - ClinPath & POCUS)
