...
- 呼叫內建的 Python 腳本 `scripts/rank_diagnostics.py`。
  ...
  ```
- 支援的情境 (Presentations)：
  - `acute_vomiting_diarrhoea` (急性嘔吐下痢 - Choosing Wisely 實裝)
  - `severe_anaemia_pallor` (嚴重貧血蒼白)
  - `pu_pd_polyuria_polydipsia` (多渴多尿 - 包含無症狀菌尿警告)
  - `icterus_hepatobiliary` (黃疸肝膽)
  - `pruritus_skin_lesions` (皮膚搔癢)
  - `skin_mass_nodule` (皮膚腫塊)
  - `feline_weight_loss_polyphagia` (高齡貓體重流失)
  - `acute_collapse_seizures` (休克癲癇)
  - `cavitary_effusion` (體腔積液)
  - `mitral_valve_disease_mmvd` (心臟瓣膜病 - Choosing Wisely ACE-I 警告實裝)
  - `osteoarthritis_chronic_pain` (骨關節炎與慢性疼痛 - Choosing Wisely 葡萄糖胺警告實裝)
